...
var product_suggestion_xhr = null;
...
$(document).ready(function () {
    ...
    // Full screen toggle
    $(document).on('click', 'button#full_screen', function (e) {
        e.preventDefault();
    ...
    $('table#pos_table tbody').on('change', 'input.pos_quantity', function () {
        ...
        // Protective check: if quantity becomes 0 or invalid, restore to 1
        ...
        updateLineTotalText(tr, line_total);
    ...
    $('button#pos-finalize-quotation').click(function () {
        //Check if product is present or not.
        if ($('table#pos_table tbody').find('.product_row').length <= 0) {
            toastr.warning(LANG.no_products_added);
            return false;
    ...
    //on save mpesa details
    $('button#pos-save-mpesa').click(function () {
        $('input#transaction_no_1_0').val($('#mpesa_transaction_no').val());
    ...
    $('button#posEditDiscountModalUpdate').click(function () {
        ...
        var discountValue = $('input#discount_amount_modal').val();
        var discountType = $('select#discount_type_modal').val();
        ...
        // Check max discount if set
        var max_value = parseFloat($('input#discount_amount_modal').data('max-discount'));
        if (discountValue != '' && !isNaN(max_value)) {
            if (discountType == 'fixed') {
                var subtotal = get_subtotal();
                max_value = __calculate_amount('percentage', max_value, subtotal);
            }
            if (parseFloat(discountValue) > max_value) {
                toastr.error($('input#discount_amount_modal').data('max-discount-error_msg') || 'Discount exceeds maximum allowed');
                return false;
            }
        }

        var distribute_discount = $('#distribute_discount_checkbox').is(':checked');

        //Clear product discounts if distributing
        if (distribute_discount) {
            clear_product_discounts();
        }
        ...
        //Update values
        $('input#discount_type').val(discountType);
        __write_number($('input#discount_amount'), parseFloat(discountValue));
        ...
        // Distribute discount evenly across products if checkbox is checked
        if (distribute_discount) {
            distribute_discount_to_products();
            // Clear the global discount so it isn't applied twice
            $('input#discount_type').val('fixed');
            __write_number($('input#discount_amount'), 0);
            $('#distribute_discount_checkbox').prop('checked', false);
        }
    ...
    // Make discount row clickable for inline editing
    // Discount row click is handled by existing modal trigger

    // Make discount row clickable for inline editing
    $(document).on('click', '#discount_row', function(e) {
        if ($(e.target).closest('input, select').length === 0) {
            console.log('=== Discount row clicked ===');

            // Check if modal exists
            if ($('#posEditDiscountModal').length === 0) {
                console.error('Discount modal element not found!');
                toastr.error('Discount modal not found in DOM');
                return;
            }
            ...
            // Debug: Check modal content after delay
            setTimeout(function() {
                console.log('Checking modal content...');
                var $modalContent = $('#posEditDiscountModal .modal-body');

                if ($modalContent.length === 0) {
                    console.error('Modal body is empty!');
                    console.log('Modal HTML:', $('#posEditDiscountModal').html());
                    return;
                }

                // Check for discount type (should exist)
                var $discountType = $('#discount_type_modal');
                console.log('discount_type_modal exists:', $discountType.length);

                // Check for discount amount input (might be missing)
                var $discountAmount = $('#discount_amount_modal');
                console.log('discount_amount_modal exists:', $discountAmount.length);

                // If discount amount input is missing, try to create it
                if ($discountAmount.length === 0) {
                    console.log('Discount amount input not found, attempting to create...');
                    // Find the form-group div that contains discount type and add amount input after it
                    var $typeFormGroup = $discountType.closest('.form-group');
                    if ($typeFormGroup.length > 0) {
                        var $newCol = $('<div class="col-md-6">' +
                            '<div class="form-group">' +
                            '<label for="discount_amount_modal">Discount Amount:</label> ' +
                            '<div class="input-group">' +
                            '<span class="input-group-addon">Ksh</span> ' +
                            '<input type="text" class="form-control" id="discount_amount_modal" name="discount_amount_modal" placeholder="0.00">' +
                            '</div></div></div>');
                        // Insert after the existing col-md-6
                        $typeFormGroup.closest('.col-md-6').after($newCol);
                        console.log('Created discount amount input');
                    }
                }

                // Log all form elements found
                console.log('Form elements in modal:');
                $('#posEditDiscountModal input, #posEditDiscountModal select, #posEditDiscountModal label').each(function() {
                    console.log(' - Element:', this.tagName, 'ID:', this.id, 'Type:', this.type, 'Display:', window.getComputedStyle(this).display);
                });

                // Check for expected elements
                console.log('discount_type_modal exists:', $('#discount_type_modal').length);
                console.log('discount_amount_modal exists:', $('#discount_amount_modal').length);
                console.log('distribute_discount_checkbox exists:', $('#distribute_discount_checkbox').length);
            }, 300);
        }
    });

    // Make tax row clickable for inline editing
    $(document).on('click', '#tax_row', function(e) {
        if ($(e.target).closest('input, select').length === 0) {
            $('#posEditOrderTaxModal').modal('show');
        }
    });
...
// Helper function to update line total text with discount display
function updateLineTotalText(tr, lineTotal) {
    ...
    var displayText = 'Ksh ' + __currency_trans_from_en(lineTotal, false);

    // Show discount if applied
    if (row_discount_amount > 0) {
        var discountInfo = '';
        if (row_discount_type == 'fixed') {
            discountInfo = ' (-' + __currency_trans_from_en(row_discount_amount, false) + ')';
        } else {
            discountInfo = ' (-' + row_discount_amount + '%)';
        }
        displayText += discountInfo;
    }

    tr.find('span.pos_line_total_text').text(displayText);
}
...
function get_product_suggestion_list(category_id, brand_id, location_id, url = null, is_enabled_stock = null, repair_model_id = null) {
    ...
    if (product_suggestion_xhr) {
        product_suggestion_xhr.abort();
    }

    product_suggestion_xhr = $.ajax({
        method: 'GET',
        url: url,
        data: {
...
function pos_insert_product_row(result) {
    ...
    updateLineTotalText(this_row, line_total);
...
function pos_total_row() {
    ...
    $('span.price_total').html('Ksh ' + __currency_trans_from_en(price_total, false));
    calculate_billing_details(price_total, total_quantity);
...
//Calculate total discounts applied to individual product rows
function get_total_product_discounts() {
    ...
    $('table#pos_table tbody tr').each(function () {
        ...
        if (row_discount_amount && row_discount_amount > 0) {
            var unit_discount = 0;
            if (row_discount_type == 'fixed') {
                unit_discount = row_discount_amount;
            } else {
                unit_discount = __calculate_amount('percentage', row_discount_amount, unit_price);
            }
            total_product_discount += unit_discount * qty;
        }
    });
...
//Distribute total discount proportionally across all products based on price
function distribute_discount_to_products() {
    var subtotal = get_subtotal();
    var total_discount = pos_discount(subtotal);
    if (total_discount <= 0 || subtotal <= 0) return;
    ...
    $('table#pos_table tbody tr').each(function () {
        ...
        if (line_total > 0) {
            var row_discount_share = (line_total / subtotal) * total_discount;
            var unit_discount = row_discount_share / qty;

            row.find('select.row_discount_type').val('fixed');
            __write_number(row.find('input.row_discount_amount'), parseFloat(unit_discount.toFixed(4)));
        } else {
            row.find('select.row_discount_type').val('fixed');
            __write_number(row.find('input.row_discount_amount'), 0);
        }
        
        // Trigger the change event so the POS system naturally updates the line totals and UI
        row.find('input.row_discount_amount').trigger('change');
    });
}

//Clear all product discounts (used when removing distributed discount)
function clear_product_discounts() {
    $('table#pos_table tbody tr').each(function () {
        var row = $(this);
        row.find('select.row_discount_type').val('fixed');
        __write_number(row.find('input.row_discount_amount'), 0);
...
function calculate_billing_details(price_total, total_quantity) {
    total_quantity = total_quantity || 0;
    var product_discounts = get_total_product_discounts();

    // We calculate a gross display subtotal so the UI shows the pre-discount sum.
    var display_subtotal = price_total + product_discounts;

    // Update product discounts display
    if ($('#product_discounts_row').length) {
        if (product_discounts > 0) {
            $('#product_discounts_amount').text('-' + __currency_trans_from_en(product_discounts, false));
            $('#product_discounts_row').show();
        } else {
            $('#product_discounts_row').hide();
        }
    }

    var discount = pos_discount(price_total);
    var combined_discount = discount + product_discounts;

    if ($('#reward_point_enabled').length) {
        total_customer_reward = $('#rp_redeemed_amount').val();
        discount = parseFloat(discount) + parseFloat(total_customer_reward);
        combined_discount = parseFloat(combined_discount) + parseFloat(total_customer_reward);

        if ($('input[name="is_direct_sale"]').length <= 0) {
            $('span#total_discount').text(__currency_trans_from_en(combined_discount, false));
        }
    }
    ...
    if ($('#types_of_service_id').length > 0 &&
        ...
        $('#packing_charge_text').text('Ksh ' + __currency_trans_from_en(packing_charge, false));
    }
    ...
    $('span#total_payable').text('Ksh ' + __currency_trans_from_en(shown_total, false));

    $('span.total_payable_span').text(__currency_trans_from_en(total_payable_rounded, true));

    // Update redesigned POS summary displays
    if ($('#subtotal_display').length) {
        $('#subtotal_display').text('Ksh ' + __currency_trans_from_en(display_subtotal, false));
    }
    if ($('#discount_display').length) {
        $('#discount_display').text('- ' + __currency_trans_from_en(combined_discount, true));
    }
    if ($('#tax_display').length) {
        $('#tax_display').text(__currency_trans_from_en(order_tax, true));
    }
    if ($('#total_payable').length) {
        $('#total_payable').text('Ksh ' + __currency_trans_from_en(shown_total, false));
        // Add pulse animation
        $('#total_payable').addClass('updated');
        setTimeout(function () {
            $('#total_payable').removeClass('updated');
        }, 300);
    }
    // Update item count badge
    if ($('#total_quantity_badge').length) {
        var qty_text = total_quantity + ' Item' + (total_quantity !== 1 ? 's' : '');
        $('#total_quantity_badge').text(qty_text);
    }

    // Update discount row display - always show so user can click to add/edit
    if ($('#discount_row').length) {
        var discountType = $('#discount_type').val();
        var discountAmt = __read_number($('#discount_amount'));
        if (combined_discount > 0) {
            var discountTypeText = '';
            if (discount > 0) {
                 discountTypeText = discountType === 'percentage' ? '(' + discountAmt + '%)' : '(Fixed)';
            } else if (product_discounts > 0) {
                 discountTypeText = '(Distributed)';
            }
            $('#discount_type_display').text(discountTypeText);
            $('#discount_amount_display').text('-' + __currency_trans_from_en(combined_discount, false));
            $('#discount_row').css('background', 'rgba(220, 38, 38, 0.05)');
        } else {
            $('#discount_type_display').text('(Click to add)');
            $('#discount_amount_display').text('0.00');
            $('#discount_row').css('background', 'transparent');
        }
        $('#discount_row').show();
    }

    // Update tax row display
    if ($('#tax_row').length) {
        var taxRate = __read_number($('#tax_calculation_amount'));
        if (order_tax > 0) {
            $('#tax_percent_display').text(taxRate > 0 ? '(' + taxRate + '%)' : '');
            $('#tax_amount_display').text(__currency_trans_from_en(order_tax, false));
            $('#tax_row').css('background', 'rgba(100, 116, 139, 0.05)');
        } else {
            $('#tax_percent_display').text('(Click to add)');
            $('#tax_amount_display').text('0.00');
            $('#tax_row').css('background', 'transparent');
        }
        $('#tax_row').show();
    }
...
$('table#pos_table tbody').on('change', 'input.pos_line_total', function () {
    ...
    updateLineTotalText(tr, subtotal);
