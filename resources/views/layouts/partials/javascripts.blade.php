...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $(document).on('click', '#check_mpesa_balance_btn', function(e) {
            e.preventDefault();
            
            Swal.fire({
                title: 'Check M-Pesa Balance',
                text: "This will initiate a balance inquiry with Safaricom. Do you want to proceed?",
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#d33',
                confirmButtonText: 'Yes, Check Balance'
            }).then((result) => {
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ route("mpesa.check-balance") }}',
                        method: 'POST',
                        dataType: 'json',
                        success: function(response) {
                            if (response.success) {
                                Swal.fire(
                                    'Inquiry Sent!',
                                    'The balance inquiry request has been sent to M-Pesa. You will receive an SMS once processed.',
                                    'success'
                                );
                            } else {
                                Swal.fire(
