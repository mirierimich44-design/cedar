...
<div class="table-responsive purchase-lines-scroll" role="region" aria-label="Purchase lines">
    ...
    id="purchase_entry_table" style="width:100%; min-width:1100px;">
        <thead>
              <tr>
                ...
                <th style="width:90px;">@if(empty($is_purchase_order)) Qty @else @lang( 'lang_v1.order_quantity' ) @endif</th>
                ...
                <th class="@if(!session('business.enable_editing_product_from_purchase') || !empty($is_purchase_order)) hide @endif" style="width:100px;">
                    @lang( 'lang_v1.profit_margin' )
                </th>
                @if(empty($is_purchase_order))
                    <th class="@if(!session('business.enable_editing_product_from_purchase')) hide @endif" style="width:120px;">
    ...
    @foreach($purchase->purchase_lines as $purchase_line)
        <tr data-row="{{ $loop->index }}"
            ...
            <td class="text-left" style="padding:12px 14px; vertical-align:middle;">
                <strong style="font-size:13px; font-weight:700; color:#0f172a;">{{ $purchase_line->product->name }}</strong>
                <div class="text-muted" style="font-size:11px; margin-top:2px;">{{$purchase_line->variations->sub_sku}}</div>
                @if( $purchase_line->product->type == 'variable')
                    <div class="text-muted" style="font-size:11px;"><b>{{ $purchase_line->variations->product_variation->name}}</b> : {{ $purchase_line->variations->name}}</div>
                @endif
                <div class="details-summary-badge" style="margin-top:6px;">
                    @php
                        $edit_sp_badge = null;
