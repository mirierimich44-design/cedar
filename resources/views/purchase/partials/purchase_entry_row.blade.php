@foreach( $variations as $variation)
    <tr @if(!empty($purchase_order_line)) data-purchase_order_id="{{$purchase_order_line->transaction_id}}" @endif @if(!empty($purchase_requisition_line)) data-purchase_requisition_id="{{$purchase_requisition_line->transaction_id}}" @endif>
        ...
        @endphp
        {{-- Hidden tds keep header/column alignment for the two hidden <th> columns --}}
        <td class="hide"></td>
        <td class="hide"></td>
        <td>
            <div style="display:flex;gap:4px;align-items:center;">
                ...
                {{-- Disc% inline --}}
                <div class="input-group input-sm" style="width:120px;flex-shrink:0;">
                    <span class="input-group-addon" style="padding:4px 5px;font-size:11px;">Disc%</span>
            ...
            @if(!empty($last_purchase_line))
                <small class="text-muted" style="font-size:10px;">
                    Prev: @format_currency($last_purchase_line->pp_without_discount)
                    &nbsp;|&nbsp;Disc: {{@num_format($last_purchase_line->discount_percent)}}%
                </small>
            @endif
            {{-- Computed unit cost after discount — hidden, updated by JS, submitted with form --}}
            {!! Form::hidden('purchases[' . $row_count . '][purchase_price]',
            number_format($purchase_price, $currency_precision, $currency_details->decimal_separator, $currency_details->thousand_separator), ['class' => 'purchase_unit_cost input_number']); !!}
        </td>
        <td>
        ...
        <td>
            <div class="tw-flex tw-flex-col tw-gap-2">
                <div class="input-group">
                    ...
                    {!! Form::text('purchases[' . $row_count . '][item_tax_percent]', 0, ['class' => 'form-control input-sm row_tax_percent input_number', 'placeholder' => 'Tax %']); !!}
                </div>
                <div class="input-group hide">
                    <span class="input-group-addon">Amt</span>
                    {!! Form::text('purchases[' . $row_count . '][item_tax]', 0, ['class' => 'form-control input-sm row_tax_amount input_number purchase_product_unit_tax', 'placeholder' => 'Tax Amount']); !!}
                </div>
            </div>
