        ...
        <div class="table-responsive">
          	<table class="table table-slim">
				<tbody>
					...
					<!-- Payments and Balances (Hide for Walk-In) -->
					@if(trim($receipt_details->customer_name) != 'Walk-In Customer')
						<!-- Payments -->
						@if(!empty($receipt_details->payments))
							@foreach($receipt_details->payments as $payment)
								<tr>
									<th>{{$payment['method']}} ({{$payment['date']}}):</th>
						...
						@if(!empty($receipt_details->total_previous_due) && !empty($receipt_details->prev_bal_label))
						<tr>
							<th>
...
@if(!empty($receipt_details->etims_qr_url))
<div class="row" style="color: #000000 !important; margin-top: 10px;">
	<div class="col-xs-12 text-center">
		<p><strong>eTIMS DETAILS</strong></p>
		<img src="{{$receipt_details->etims_qr_url}}" style="width: 100px; height: 100px;">
		<p style="font-size: 10px; margin-top: 5px;">
			Invoice No: {{$receipt_details->etims_invoice_number}}<br>
			Control Code: {{$receipt_details->etims_signature}}
		</p>
	</div>
</div>
@endif