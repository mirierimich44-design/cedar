<div class="{{$class ?? ''}} tw-mb-4 tw-transition-all tw-duration-200 tw-bg-white tw-shadow-sm tw-rounded-xl tw-ring-1 hover:tw-shadow-md tw-ring-gray-200"
    ...
    @if (empty($header))
        @if (!empty($title) || !empty($tool))
            <div style="background:linear-gradient(135deg,var(--theme-dark) 0%,var(--theme-main) 100%); padding:12px 16px; display:flex; align-items:center; justify-content:space-between; gap:10px;">
