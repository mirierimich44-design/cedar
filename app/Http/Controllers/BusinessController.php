...
class BusinessController extends Controller
{
    ...
    */
    ...
    public function getBusinessSettings()
    {
        ...
        // Custom feature toggles (BusinessFeatureSetting)
        $ip_restriction_enabled = \App\Models\BusinessFeatureSetting::isEnabled('ip_restriction', $business_id);

        return view('business.settings', compact('business', 'currencies', 'tax_rates', 'timezone_list', 'months', 'accounting_methods', 'commission_agent_dropdown', 'units_dropdown', 'date_formats', 'shortcuts', 'pos_settings', 'modules', 'theme_colors', 'email_settings', 'sms_settings', 'whatsapp_settings', 'mail_drivers', 'allow_superadmin_email_settings', 'custom_labels', 'common_settings', 'weighing_scale_setting', 'payment_types', 'enabled_modules', 'login_settings', 'ip_restriction_enabled'));
    }
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            $business_details = $request->only(['name', 'start_date', 'currency_id', 'tax_label_1', 'tax_number_1', 'tax_label_2', 'tax_number_2', 'default_profit_percent', 'default_sales_tax', 'default_sales_discount', 'sell_price_tax', 'sku_prefix', 'time_zone', 'fy_start_month', 'accounting_method', 'transaction_edit_days', 'sales_cmsn_agnt', 'item_addition_method', 'currency_symbol_placement', 'on_product_expiry',
                ...
                'time_format', 'ref_no_prefixes', 'theme_color', 'font_family', 'email_settings',
                ...
                'digitax_api_key', 'etims_sync_mode', 'etims_tpin', 'etims_enabled', ]);
            ...
            $business_details['etims_enabled'] = $request->input('etims_enabled') == 1 ? 1 : 0;
    ...
    public function testWhatsAppConfiguration(Request $request)
    {
        try {
            ...
            // Remove any non-numeric characters except + from the start
            $test_number = preg_replace('/[^0-9+]/', '', $test_number);
            if (strpos($test_number, '+') !== 0) {
                $test_number = '+' . $test_number;
            }

            $message = 'This is a test message from MichPOS WhatsApp Integration.';

            if ($provider == 'meta') {
                $response = $this->sendMetaWhatsAppMessage(
                    $request->input('access_token'),
                    $request->input('phone_number_id'),
                    $request->input('api_url', 'https://graph.facebook.com/v17.0'),
                    $test_number,
                    $message
                );
            } elseif ($provider == 'twilio') {
                $response = $this->sendTwilioWhatsAppMessage(
                    $request->input('twilio_sid'),
                    $request->input('twilio_token'),
                    $request->input('twilio_from'),
                    $test_number,
                    $message
                );
            } else {
                $response = $this->sendCustomWhatsAppMessage(
                    $request->input('custom_url'),
                    $request->input('custom_api_key'),
                    $request->input('custom_sender'),
                    $test_number,
                    $message
                );
            }
            ...
            if ($response['success']) {
                return response()->json([
                    'success' => true,
                    'msg' => __('lang_v1.whatsapp_test_sent_successfully'),
                ]);
            } else {
                return response()->json([
                    'success' => false,
                    'msg' => $response['error'] ?? __('lang_v1.whatsapp_test_failed'),
                ]);
            }
        } catch (\Exception $e) {
     ...
     */
    private function sendMetaWhatsAppMessage($access_token, $phone_number_id, $api_url, $to, $message)
    {
        if (empty($access_token) || empty($phone_number_id)) {
            return ['success' => false, 'error' => 'Access token and Phone Number ID are required'];
        }

        $url = rtrim($api_url, '/') . '/' . $phone_number_id . '/messages';
     ...
     */
    private function sendTwilioWhatsAppMessage($sid, $token, $from, $to, $message)
    {
        if (empty($sid) || empty($token) || empty($from)) {
            return ['success' => false, 'error' => 'Twilio SID, Token, and From number are required'];
        }
     ...
     */
    private function sendCustomWhatsAppMessage($url, $api_key, $sender, $to, $message)
    {
        if (empty($url)) {
