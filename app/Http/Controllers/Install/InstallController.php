...
class InstallController extends Controller
{
    ...
    public function postDetails(Request $request)
    {
        ...
        try {
            ...
            // Mock missing fields
            $input['ENVATO_PURCHASE_CODE'] = 'mock_purchase_code';
            $input['ENVATO_EMAIL'] = 'mock@example.com';
            $input['ENVATO_USERNAME'] = 'mock_username';
            $input['MAIL_MAILER'] = 'log';
            $input['MAIL_FROM_ADDRESS'] = 'admin@example.com';
            $input['MAIL_FROM_NAME'] = $input['APP_NAME'];
            $input['MAIL_HOST'] = 'smtp.mailtrap.io';
            $input['MAIL_PORT'] = '2525';
            $input['MAIL_ENCRYPTION'] = 'tls';
            $input['MAIL_USERNAME'] = null;
            $input['MAIL_PASSWORD'] = null;
            ...
            try {
                ...
                // If connection failed, check if it's because of unknown database
                if (str_contains($e->getMessage(), "Unknown database")) {
                     try {
                        // Connect without database
                         $link = @mysqli_connect($input['DB_HOST'], $input['DB_USERNAME'], $input['DB_PASSWORD'], null, $input['DB_PORT']);
                         if ($link) {
                             $sql = "CREATE DATABASE " . $input['DB_DATABASE'];
                             if (mysqli_query($link, $sql)) {
                                 $mysql_link = $link; // Connection successful after creation
                             } else {
                                  throw new \Exception("Error creating database: " . mysqli_error($link));
                             }
                         } else {
                            throw $e; // Re-throw original error if can't connect at all
                         }
                     } catch (\Exception $ex) {
            ...
            if (!$mysql_link && mysqli_connect_errno()) {
