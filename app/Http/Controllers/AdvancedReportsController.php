        ...
        try {
            if (Schema::hasTable('mpesa_transactions')) {
                $mq = DB::table('mpesa_transactions')
                    ...
                    ->whereBetween(DB::raw('DATE(COALESCE(transaction_date, created_at, updated_at))'), [$start, $end]);
                // paid/success statuses
                $mq->where(function ($w) {
                    $w->whereIn('status', ['paid', 'success', 'completed', 'Completed'])
                        ->orWhere('ResultCode', 0)
                        ->orWhere('result_code', 0);
                });
                $mpesaRows = $mq->orderByDesc('id')->limit(1000)->get();
                $mpesaTotal = (float) $mpesaRows->sum(function ($r) {
                    return (float) ($r->amount ?? $r->trans_amount ?? 0);
                });
            }
        } catch (\Throwable $e) {
