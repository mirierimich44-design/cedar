...
class ProductUtil extends Util
{
    ...
    public function getTrendingProducts($business_id, $filters = [])
    {
        ...
        $products = $query->leftjoin('variations as v', 'tsl.variation_id', '=', 'v.id')
                        ->select(
    ...
    public function filterProduct($business_id, $search_term, $location_id = null, $not_for_selling = null, $price_group_id = null, $product_types = [], $search_fields = [], $check_qty = false, $search_type = 'like')
    {
        ...
        $data = $query->groupBy('variations.id')
             ->orderByRaw('
                 CASE
                     WHEN products.enable_stock = 0 THEN 0
                     WHEN COALESCE(VLD.qty_available, 0) > 0 THEN 1
                     ELSE 2
                 END ASC
             ')
    ...
    public function getPosProductRow($variation_id, $location_id)
    {
        ...
        try {
            ...
            // Pre-check stock availability for better error messaging
            $business_id = request()->session()->get('user.business_id');
            $business_details = \App\Business::find($business_id);
            $pos_settings = empty($business_details->pos_settings) ? [] : json_decode($business_details->pos_settings, true);
            $allow_overselling = !empty($pos_settings['allow_overselling']);
            
            if (!$allow_overselling) {
                // Check if product exists and has stock at this location
                $variation = \App\Variation::with('product')->find($variation_id);
                
                if (!$variation) {
                    $output['success'] = false;
                    $output['msg'] = __('lang_v1.product_not_found');
                    return $output;
                }
                
                $product = $variation->product;
                
                // Only check stock if stock tracking is enabled for this product
                if ($product->enable_stock == 1) {
                    $stock_record = \App\VariationLocationDetails::where('variation_id', $variation_id)
                        ->where('location_id', $location_id)
                        ->first();
                    
                    if (!$stock_record) {
                        $output['success'] = false;
                        $output['msg'] = __('lang_v1.item_out_of_stock') . ' (' . __('lang_v1.no_stock_at_location') . ')';
                        return $output;
                    }
                    
                    if ($stock_record->qty_available <= 0) {
                        $output['success'] = false;
                        $output['msg'] = __('lang_v1.item_out_of_stock') . ' (0 ' . __('lang_v1.available') . ')';
                        return $output;
                    }
                }
            }
            ...
            if (empty($output)) {
                $output['success'] = false;
                $output['msg'] = __('lang_v1.item_out_of_stock') . ' (Variation details not found)';
                return $output;
            }
            ...
            $output['success'] = false;
            $output['msg'] = __('lang_v1.item_out_of_stock') . ' [Error: ' . $e->getMessage() . ']';
        }
