...
class NotificationUtil extends Util
{
    ...
    public function sendAfricasTalkingWhatsapp($to, $message)
    {
        $username = config('services.africastalking.username');
        $apiKey = config('services.africastalking.api_key');
        $from = config('services.africastalking.whatsapp_from');

        if (empty($username) || empty($apiKey) || empty($from)) {
            \Log::error('AfricasTalking credentials missing in config/services.php');
            return false;
        }
        ...
        try {
            ...
            \Log::info('AfricasTalking WhatsApp Response: ', $result);

            return $result;
        } catch (\Exception $e) {
            \Log::emergency('AfricasTalking WhatsApp Error: ' . $e->getMessage());
            return false;
        }
    }
}
