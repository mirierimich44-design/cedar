...
class TransactionUtil extends Util
{
        ...
        foreach ($payments as $payment) {
            ...
            if (! empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    if ($payment['method'] == 'kcb_buni') {
                        $payment_data['transaction_no'] = $payment['kcb_buni_reference'] ?? null;
                    }
    ...
    public function editPaymentLine($payment, $transaction = null, $uf_data = true)
    {
        ...
        if ($payment['method'] == 'kcb_buni') {
            $payment['transaction_no'] = $payment['kcb_buni_reference'] ?? ($payment['transaction_no'] ?? null);
        }
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        $output['etims_invoice_number'] = $transaction->etims_invoice_number;
        $output['etims_qr_url'] = $transaction->etims_qr_url;
        $output['etims_signature'] = $transaction->etims_signature;
        ...
        if (! empty($output['taxes'])) {
            ...
            $output['total_tax_summary'] = $this->num_f($total_tax + $transaction->tax_amount, $show_currency, $business_details);
        } else {
            $output['total_tax_summary'] = $this->num_f($transaction->tax_amount, $show_currency, $business_details);
        }
