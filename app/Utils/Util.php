...
class Util
{
    ...
    public function payment_types($location = null, $show_advance = false, $business_id = null)
    {
        ...
        $payment_types = ['cash' => __('lang_v1.cash'), 'card' => __('lang_v1.card'), 'cheque' => __('lang_v1.cheque'), 'bank_transfer' => __('lang_v1.bank_transfer'), 'mpesa' => 'M-Pesa', 'pesapal' => 'Pesapal', 'kcb_buni' => 'KCB Buni', 'other' => __('lang_v1.other')];

        $payment_types['custom_pay_1'] = ! empty($custom_labels['payments']['custom_pay_1']) ? $custom_labels['payments']['custom_pay_1'] : 'MPESA';
        ...
        if (! empty($location)) {
            ...
            foreach ($payment_types as $key => $value) {
                if (! in_array($key, $enabled_accounts) && $key !== 'custom_pay_1') {
    ...
    public function getContactDue($contact_id, $business_id = null)
    {
        ...
        $due = ($contact_payments->total_invoice ?? 0)
             + ($contact_payments->total_purchase ?? 0)
             - ($contact_payments->total_paid ?? 0)
             - ($contact_payments->purchase_paid ?? 0)
             + ($contact_payments->opening_balance ?? 0)
             - ($contact_payments->opening_balance_paid ?? 0)
             - ($contact_payments->total_sell_return ?? 0)
             + ($contact_payments->sell_return_paid ?? 0);
