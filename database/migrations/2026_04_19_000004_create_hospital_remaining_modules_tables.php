...
class CreateHospitalRemainingModulesTables2026 extends Migration
{
    public function up()
    {
        // --- 1. Physiotherapy ---
        ...
        Schema::create('hospital_mortuary_records', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('business_id')->unsigned();
            $table->string('body_name')->nullable(); // Can be null if unknown/linked to patient
            $table->integer('patient_id')->unsigned()->nullable();
            $table->dateTime('admitted_at');
            $table->dateTime('released_at')->nullable();
            $table->string('relative_name')->nullable();
            $table->string('relative_phone')->nullable();
            $table->string('storage_location')->nullable(); // e.g., Fridge 1, Slot A
