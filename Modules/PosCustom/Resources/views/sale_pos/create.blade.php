...
@section('content')
    ...
    <section class="{{-- content --}} no-print">
        ...
        <pos-items-totals class = "tw-bg-white grid-cols-1 grid   @if ($poscustom_posi_totals == 'right' ) md:col-start-2  {{-- @else grid-rows-[auto,1fr]  --}} @endif md:row-start-2 md:block tw-rounded-lg" >
            
            <div class="flex @if ($poscustom_style_totals == 's1') {{-- buttons on the left --}} tw-h-dvh-totals-left @else tw-h-dvh-totals-fixed @endif 
        ...
        <pos-products class="@if($poscustom_width_totals == 'full') hidden @endif @if ($poscustom_posi_totals == 'right' ) md:col-start-1 {{-- grid-rows-[1fr,auto] --}} @endif md:row-start-2  {{--bg-white p-2 --}} "> <!--p-2 padding for the  div products -->    
            @if (empty($pos_settings['hide_product_suggestion']) && !isMobile())
                <!--Including diferents style in the right side-->
                    ...
                    @if ($poscustom_style_cate == 's1') {{-- category on the left --}}
