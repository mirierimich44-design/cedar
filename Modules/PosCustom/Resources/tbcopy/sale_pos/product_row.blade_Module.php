...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	...
	<td style="width: auto;">
		...
		@if (!empty($pos_settings['poscustom_show_imgproduct']))
		...
		<div class="tw-flex tw-inline">	
			...
			<div class="input-group input-number ">
				<span class="input-group-btn"><button type="button" class="btn btn-default btn-flat quantity-down"><i class="fa fa-minus text-danger"></i></button></span>
					<input type="text" data-min="1" style="@if (isMobile()) width: 50px; @else width: 100%; @if (empty($pos_settings['poscustom_show_units'])) width: auto; @endif  @endif"
			...
			{{--$pos_settings['poscustom_show_units']--}}	

			@if (!empty($pos_settings['poscustom_show_units']))
				<input type="hidden" name="products[{{$row_count}}][product_unit_id]" value="{{$product->unit_id}}"
				style="width: 30%;" > 
				@if(count($sub_units) > 0)
					<br>
					<select name="products[{{$row_count}}][sub_unit_id]" class="form-control input-md {{-- input-sd --}} sub_unit">
						@foreach($sub_units as $key => $value)
