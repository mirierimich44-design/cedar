    ...
    var global_arrCateSuca = {cate: null, suca: null};
...
$(document).ready(function() {
    ...
    $('select#select_location_id').on('change', function(e) {
        ...
        var location_id = $('input#location_id').val();
            ...
            if (global_arrCateSuca.cate)
            {
                //#JCN Current category ON in the tab
                change_status_tab(global_arrCateSuca.cate, 1);
            }
    ...
    $('button#btn_product_category').on('click', function(e) {
        ...
        change_status_tab(global_arrCateSuca.cate, 0);
        change_status_tab(global_arrCateSuca.suca, 0);
        //toastr.warning("btn category : " + global_p_category_id);
   
        //Current tab category
        global_arrCateSuca.cate = $(this).val();
        //#JCN Current category ON in the tab
        change_status_tab(global_arrCateSuca.cate, 1)
...
/*         change_status_tab(maincategory, 1); */
        
        //#JCN Previous category OFF in the tab
        change_status_tab(global_arrCateSuca.cate, 0);
        change_status_tab(global_arrCateSuca.suca, 0);
        
        //#JCN Save the current cate and suca in the global arr
        //In this case i change the value of suca & cate to follow the logic in button "NEXT" 
        global_arrCateSuca = {
            cate: $(this).data('value'), /*In the cate i save the suca*/
            suca: $(this).data('maincategory') /*In the suca i save the cate*/
        };
        ...
        //#JCN Current category ON in the tab
        change_status_tab(global_arrCateSuca.cate, 1)
        change_status_tab(global_arrCateSuca.suca, 1)
        ...
        if (location_id != '' || location_id != undefined) {
            get_product_suggestion_list(
                global_arrCateSuca.cate, /*The filter is by the subcategoria in this case*/
                global_brand_id,
                $('input#location_id').val(),
                null
            );
    ...
    $('.main-category').on('click', function(){
        ...
        //#JCN Save the current cate and suca in the global arr
        global_arrCateSuca = {
            cate: $(this).data('value'),
            suca: $(this).data(null)
        };        

        //#JCN Current category ON in the tab cse only with categoria
        change_status_tab(global_arrCateSuca.cate, 1)
        //change_status_tab(global_arrCateSuca.suca, 1)
...
function change_status_tab(in_category_id, flag )
    {
        //return;
        ...
        arrow = document.getElementsByClassName('arrow-inactive'); //Color of the iocn to close/open suca
        ...
        if (el[0] ) //Use for the quick menu & left catergories
        {
            
            //#Flag ON=1 OFF=0
            if (flag == 1){
                //Case category bar
                el[0].classList.add(theme_tab_g, theme_tab_f);
                el[0].style.color = 'white';
                el[0].style.border = "2px solid white";
            }else{
                //Case category bar
                el[0].classList.remove(theme_tab_g, theme_tab_f);   
                el[0].style.backgroundColor = '#ffffff';
                el[0].style.color = 'black';
                el[0].style.border = ""; 
               
            }
            //console.log('Entrando arreglo [0]...' + ' theme_tab_g: ' + theme_tab_g + ' ' + theme_tab_f);
        }
            //Exist 2 item in the array used for the quick menu categories
            //console.log('Entrando arreglo [1]...' + ' theme_tab_g: ' + theme_tab_g + ' ' + theme_tab_f);
            if (el[1]){
                //#Flag ON=1 OFF=0
                if (flag == 1){
                    //Case category menu
                    el[1].classList.add(theme_tab_g, theme_tab_f);
                    //el[1].classList.add(theme_tab_f);
                    //el[1].classList.add(theme_tab_t);
                    el[1].classList.toggle('cardpanel');
                    /* arrow[0].classList.toggle('arrow-active'); */
                    el[1].style.color = 'white';
                    el[1].style.border = "4px solid white";
                }else{
                    //Case category menu
                    el[1].classList.remove(theme_tab_g, theme_tab_f);
                    //el[1].classList.add(theme_tab_f);
                    //el[1].classList.add(theme_tab_t);                    
                    el[1].classList.toggle('cardpanel');
                    /* arrow[0].classList.toggle('arrow-active'); */
...
function get_product_suggestion_list(category_id, brand_id, location_id, url = null, is_enabled_stock = null, repair_model_id = null) {
    ...
    **/
    ...
    if (category_id == null) {
        ...
        global_arrCateSuca.cate = category_id;
        //#JCN Current category ON in the tab
        change_status_tab(global_arrCateSuca.cate, 1);                
...
//JCN To calculate the number of divs = products in the container for products
function get_number_divs () {
    ...
    */
    ...
    //Size of the DIV for products
    //#JCN Testing 2025 New Method  the width_product is set in settings 
    
    const width_Div = $('input#ps_width_product').val();
    //width_Div = 109;
    //const heightDiv = $('input#ps_height_product').val(); //To implement in the future
    const height_Div = 109;

    /*#JCN build a scale for the product to determinate the pagination*/
    const elem_div_main = document.getElementById('product_list_body'); //The container "class product_lis_body" for products     
    const widthContainer = elem_div_main.offsetWidth;
    const heightContainer = elem_div_main.offsetHeight;
   
    const divsByLine = Math.floor(widthContainer / height_Div); //Take the height to calculate x lines
    const divsByColumn = Math.floor(heightContainer / width_Div); //Take the width

    const w_paginate = divsByColumn * divsByLine;
      
    //toastr.info('anchoDiv: '+ width_Div + 'widthContainer: ' + widthContainer + 'heightContainer: ' + heightContainer + 'Filas: ' + divsByLine + ' Columnas: ' + divsByColumn + ' = ' + w_paginate);
    
    
    //#JCN 2024 old Method 
    //const w = elem_div_main.clientWidth;
    //const h = elem_div_main.clientHeight;
    //const w_paginate = ( Math.round(w/width_Div)* Math.round(h/width_Div)); //Determina # de elementos a mostrar en el div
    //const w_paginate = ( Math.trunc(w/width_Div)* Math.trunc(h/width_Div)); //Determina # de elementos a mostrar en el div
    //console.log('w_paginate = ' + w_paginate); 
    //const w_paginate = ( Math.round((w*h)/(width_Div*width_Div)));       
    //toastr.info('WSize:[ W=' + w + '*H=' + h +' ] ('+width_Div  + ') Paginate:[' + w_paginate + ']' );
...
$('div#product_list_body1').on('scroll', function() {
    if ($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight) {
        ...
        get_product_suggestion_list(global_arrCateSuca.cate, brand_id, location_id, null, is_enabled_stock, device_model_id);
    }
});
...
$('#next').on('click',function () {
    ...
    var page = parseInt($('#suggestion_page').val());
        ...
        $('#suggestion_page').val(page);
        var location_id = $('input#location_id').val();
        var category_id = global_arrCateSuca.cate;
...
/**#JCN 2025 Add to work with accordion for categories and subcategories
 * Common Script
*/
const accItems = document.querySelectorAll(".acc-item");
    accItems.forEach(accItem => {
        accItem.addEventListener('click', () => {
            accItem.classList.toggle("accopen")  /*If exist Remove*/
            accItem.classList.toggle("accclose") /*If no exist Add*/      
            if (accItem.classList.contains("accopen")) {
            acccloseOtherAccordions(accItem)
            }
        })
    })

const acccloseOtherAccordions = (activeAcc) => {
        accItems.forEach(accItem => {
            if (accItem !== activeAcc && accItem.classList.contains("accopen")) {
            accItem.classList.toggle("accopen")
            accItem.classList.toggle("accclose")
            }
        })
    }
